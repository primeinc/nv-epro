-- Silver transformation for purchase_orders dataset v0.3.0
...
WHERE 1=1
  ...
  AND sent_date <= CURRENT_DATE + INTERVAL 30 DAY  -- Allow some future dates for pre-orders
  AND total_amount >= 0  -- No negative amounts
  -- Remove test data
  AND po_id NOT ILIKE '%TEST%'
  AND po_id NOT ILIKE '%DEMO%'

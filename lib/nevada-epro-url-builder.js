...
function buildUrl(recordId, recordType) {
  ...
  // Nevada ePro uses + for spaces (not %20)
  const encodedId = recordId.trim().replace(/ /g, '+');
  const path = pattern.template.replace('{ID}', encodedId);
...
function isValidIdFormat(recordId, recordType) {
  ...
  const patterns = {
    // PO formats have expanded to include:
    // Standard: 72DOW-NV18-6
    // R-prefix: R59848
    // IGOR format: IGOR 535881
    // Basically any non-empty string now since Nevada ePro accepts various formats
    purchase_order: /^.+$/,  // Accept any non-empty PO ID

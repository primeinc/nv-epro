    ...
    environment: {
      ...
      chromiumVersion: (() => {
        try {
          ...
          return chromium._implementation._browserPath.match(/chromium-(\d+)/)?.[1] || 'unknown';
        } catch {
          // Fallback: try to get from playwright's internal registry
          try {
            const registry = require(path.join(process.cwd(), 'node_modules', 'playwright-core', 'lib', 'server', 'registry'));
            return registry.registry.chromium()[0].revision || 'unknown';
          } catch {
            return 'unknown';
          }
        }

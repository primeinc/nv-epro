...
async function ingestToBronze(csvPath, dataset, options = {}) {
  ...
  const runId = options.runId || generateRunId(eventTime, csvPath + fileHash);
  ...
  if (await bronzeExists(dataset, fileHash, bronzeBasePath)) {
    ...
    // Update manifest with this run ID to track which runs saw this data
    const manifestPath = path.join(path.dirname(bronzePath), 'manifest.json');
    try {
      const manifestContent = await fs.readFile(manifestPath, 'utf-8');
      const manifest = JSON.parse(manifestContent);
      
      // Initialize seen_by_runs if it doesn't exist
      if (!manifest.seen_by_runs) {
        manifest.seen_by_runs = [manifest.run_id]; // Include original run
      }
      
      // Add this run if not already tracked
      if (!manifest.seen_by_runs.includes(runId)) {
        manifest.seen_by_runs.push(runId);
        manifest.last_seen_at = new Date().toISOString();
        manifest.seen_count = manifest.seen_by_runs.length;
      ...
      console.warn(`Failed to update manifest lineage: ${error.message}`);
    }
